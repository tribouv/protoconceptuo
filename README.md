...
2. **Le plan chargé devient l’existant.** Dès qu’un plan est ouvert (exemple, PDF reconstruit, analyse IA), chaque action compte comme modification : dessiner un mur, en supprimer un, le déplacer, le couper, percer, reboucher, changer une épaisseur. Les écarts avec l’existant sont dessinés sur le plan, en jaune pour ce qui est à démolir et en rouge pour ce qui est à construire, avec un numéro de repère.
3. **Panneau Modifications** (à droite du plan) : bilan démolition / construction en m², et **journal** des actions, la plus récente en haut. Un clic sur une action centre le plan dessus ; la dernière action s’annule d’un clic (ou Cmd/Ctrl Z). **Tout annuler** ramène le plan à l’existant. Recharger un plan d’exemple remet les modifications et l’historique d’annulation à zéro.
4. **Corriger l’existant** (bas du panneau) : si l’extraction a manqué un mur, on repasse en relevé. Le plan revient à l’existant, les corrections ne comptent pas comme travaux, puis **Terminer le relevé** fige le nouvel existant. Un PDF sans tracé exploitable s’ouvre directement en relevé.
5. **Bilan détaillé** ouvre le tableau des postes : repère, type, coordonnées de début et de fin, longueur, épaisseur, hauteur, surface nette. Un clic sur une ligne zoome le plan sur le repère. L’onglet JSON donne le document d’échange avec Conceptuo, à copier ou à télécharger. Le format est décrit dans [GUIDE-DEVELOPPEURS.md](GUIDE-DEVELOPPEURS.md).
6. Les pièces et leurs surfaces sont calculées à partir des murs. Une pièce garde la surface imprimée sur le plan tant que ses murs n’ont pas changé ; sinon, sa surface est recalculée. Supprimer le mur entre deux pièces les réunit en une seule pièce, « Cuisine + Bureau » par exemple : un clic sur son nom permet de la renommer.
7. Toutes les modifications se font dans le plan 2D. La maquette 3D en est la représentation en direct : on peut la faire tourner et y cliquer pour sélectionner un élément, mais pas y déplacer quoi que ce soit.
8. Enregistrer un JSON pour reprendre l’édition, ou exporter un GLB pour un autre outil 3D.
