...
import { bounds, catalog, exampleProject, examples, length, moveEntity, rescale, round, uid, validateProject, type Item, type Point, type Project, type Tool, type Wall } from '@/lib/editor/model';
import { isPdf, readPlan, renderPlan } from '@/lib/editor/pdf';
import { finalize, mergeCandidate, mergeWalls, orientWall, placeOpeningAt, removeWall, resizeWall, setOpeningWidth, splitWall, squareCorner, type Anchor } from '@/lib/editor/edits';
...
export default function Editor() {
    const [project, setProject] = useState<Project>(exampleProject), projectRef = useRef(project);
    projectRef.current = project;
    const [selected, setSelected] = useState<string | null>(null), [tool, setTool] = useState<Tool>('select'), [revision, setRevision] = useState(0), [opacity, setOpacity] = useState(.65), [tab, setTab] = useState('plan'), [anchor, setAnchor] = useState<Anchor>('a');
    ...
    // étape 2 : écarts avec l'existant validé (voir lib/editor/changes.ts)
    const [showChanges, setShowChanges] = useState(true), [changesOpen, setChangesOpen] = useState(false), [reopenOpen, setReopenOpen] = useState(false), [focus, setFocus] = useState<{ point: Point; ref: number; key: number } | null>(null);
    ...
    const commit = useCallback((next: Project, merge?: string) => { const previous = projectRef.current, now = Date.now(), last = lastMerge.current; lastMerge.current = merge ? { key: merge, time: now } : null; if (!(merge && last && last.key === merge && now - last.time < 800))
        setPast(prev => [...prev.slice(-29), previous]); setFuture([]); setProject(next); projectRef.current = next; setSaved('Modifications…'); }, []);
    ...
    function validateExisting() { const p = projectRef.current; commit({ ...p, existing: { walls: p.walls, labels: p.labels, validatedAt: new Date().toISOString() } }); setSelected(null); toast.success('Existant validé. Chaque modification est désormais un poste de travaux.'); }
    function reopenSurvey() { commit({ ...projectRef.current, existing: null }); setReopenOpen(false); toast.info('Relevé rouvert : le plan actuel redeviendra l’existant à la validation.'); }
    ...
    function addWall(a: Point, b: Point) { const w: Wall = { id: uid(), name: `Mur ${project.walls.length + 1}`, a, b, height: 2.6, thickness: .15, openings: [] }; commit({ ...project, walls: [...project.walls, w] }); setSelected(w.id); }
    ...
    async function analyze() { if (!project.background)
        ...
        const next = { ...convertAnalysis(data, project), existing: null };
        commit(next);
 ...
 {project.existing && changes ? <div className="stage-card project"><p className="stage-step">Étape 2 · Projet de rénovation</p><p className="small-text">Démolition {changes.totals.demolishedLength.toFixed(2).replace('.', ',')} m · {changes.totals.demolishedArea.toFixed(2).replace('.', ',')} m²<br/>Construction {changes.totals.builtLength.toFixed(2).replace('.', ',')} m · {changes.totals.builtArea.toFixed(2).replace('.', ',')} m²<br/>{changes.totals.openingsCreated} percement{changes.totals.openingsCreated > 1 ? 's' : ''} · {changes.totals.openingsFilled} rebouchage{changes.totals.openingsFilled > 1 ? 's' : ''}{changes.totals.openingsModified ? ` · ${changes.totals.openingsModified} baie${changes.totals.openingsModified > 1 ? 's' : ''} modifiée${changes.totals.openingsModified > 1 ? 's' : ''}` : ''}</p><label className="stage-toggle"><input type="checkbox" checked={showChanges} onChange={e => setShowChanges(e.target.checked)}/> Afficher démolitions / constructions</label><button className="primary-button full" onClick={() => setChangesOpen(true)}><ListChecks /> Modifications ({changes.count})</button><button className="plain-button stage-link" onClick={() => setReopenOpen(true)}><RotateCcw /> Rouvrir le relevé</button></div>
 : project.walls.length > 0 && <div className="stage-card"><p className="stage-step">Étape 1 · Relevé de l’existant</p><p className="small-text">Corrigez l’extraction, puis validez. Les corrections faites avant la validation ne comptent pas comme travaux.</p><button className="primary-button full" onClick={validateExisting}><ClipboardCheck /> Valider l’existant</button></div>}
 {examples.map(ex => <button key={ex.id} className="tool muted-tool" onClick={() => { commit(ex.project()); setSelected(null); setTool('select'); setSourceFile(null); setWarnings([]); setRevision(r => r + 1); }}><Box /> {ex.label}</button>)}
 ...
 <footer className="statusbar"><span><Check size={13}/>{saved}</span><span>{project.calibrated ? '1 unité = 1 mètre' : 'Dimensions provisoires'} · Reconstruction à vérifier</span></footer></section></div>
 ...
 <Dialog open={reopenOpen} onOpenChange={setReopenOpen}><DialogContent><DialogTitle>Rouvrir le relevé de l’existant ?</DialogTitle><DialogDescription>Les modifications en cours ne seront plus comptées comme travaux : à la prochaine validation, le plan actuel deviendra l’existant. Annuler (⌘ Z) permet de revenir en arrière.</DialogDescription><div className="dialog-actions"><button className="outline-button" onClick={() => setReopenOpen(false)}>Garder le projet</button><button className="primary-button" onClick={reopenSurvey}><RotateCcw /> Rouvrir le relevé</button></div></DialogContent></Dialog>
